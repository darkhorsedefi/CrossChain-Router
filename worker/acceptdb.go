...
import (
	...
	"github.com/anyswap/CrossChain-Router/v3/leveldb"
...
const (
	identifierKey = "router-identifier"
...
func getSwapKeyPrefix(args *tokens.BuildTxArgs) string {
	return strings.ToLower(fmt.Sprintf("%s:%d:%s:%d:",
		args.SwapID, args.LogIndex, args.FromChainID.String(), args.SwapType))
}
...
func CheckAcceptRecord(args *tokens.BuildTxArgs) (err error) {
	...
	for iter.Next() {
		...
		if errt == nil && txStatus != nil && txStatus.BlockHeight > 0 { // on chain
			if receipt, ok := txStatus.Receipt.(*types.RPCTxReceipt); ok { // for eth like chain
		...
		} else if tx, err := toBridge.GetTransaction(oldSwapTx); err == nil { // in tx pool
